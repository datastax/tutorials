...
case $1 in
  ...
  tutorials)
    echo "You are building the tutorials docset."
    echo "Remove and remake the antora.yml symlinks."
    cd docs-src/tutorials-core
    rm antora.yml; ln -s antora-tutorials.yml antora.yml
    cd ../..
    echo "Run the tutorials build."
    npm run build:localtutorials
    ;;
...
#    ;;
  ...
  tutorials+adoc)
    echo "You are building the tutorials+adoc docset."
    echo "Remove and make antora.yml symlinks."
    cd ./../../adoc-howto
    rm antora.yml; ln -s antora-tutorials+adoc.yml antora.yml
    cd ../tutorials/docs/docs-src/tutorials-core
    cd ../..
    echo "Run the tutorials+adoc build."
    npm run build:localtutorials+adoc
    ;;
esac
